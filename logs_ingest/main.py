...
def is_too_old(timestamp: str, self_monitoring: SelfMonitoring, log_part: str):
    if timestamp:
        try:
            ...
            if (datetime.now(timezone.utc) - date).total_seconds() > (record_age_limit - 1440):
...
def parse_record(record: Dict, self_monitoring: SelfMonitoring):
    ...
    if content:
        ...
        if len(parsed_record["content"]) > content_length_limit:
            ...
            parsed_record["content"] = parsed_record["content"][:trimmed_len] + DYNATRACE_LOG_INGEST_CONTENT_MARK_TRIMMED
    return parsed_record
